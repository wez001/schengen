...
def test(dttt): #get total days on holiday in last 180 days ##################THIS WILL NEED TO ACOMODATE NEW DATE WHEN PUT IN
    ...
    for item in current_user.hol:
        ...
        if d1>=dtt-days180 and d2<=dtt+days180:#all safe dates that don go out of bounds
            tot_days+=d2-d1
        ...
        if d1<=dtt+days180 and d2>dtt+days180:#days on high side of date
            extra_days2=timedelta(0)
            extra_days2+=(dtt+days180)-d1
