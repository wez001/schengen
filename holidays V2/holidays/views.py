...
def home():
    if request.method == 'POST':            #if buttin pressed get holiday dates and add to database
        ...
        db.session.add(new_hol)    #add hol to db
        db.session.commit()         #commit to file
        flash('Holiday added.',category='ok')

    
                    #create a list of holidays to pass to html
                    #this is to print the items in order
    list_to_pass=[]
    for item in current_user.hol:
        list_to_pass.append((item.date1,item.date2))    
    list_to_pass=sorted(list_to_pass, key=lambda x: x[1]) #this has sorted list into order

    return render_template("home.html", user=current_user, hl=list_to_pass)
